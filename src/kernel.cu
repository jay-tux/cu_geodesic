...
template <typename T> struct cu_arr {
  // array size
  size_t size;
  // array data buffer
  T *data;
  ...
  __host__ static cu_arr<T> copy(size_t size, T *buf) {
    ...
    CUDA_SAFE_NO_RET(cudaMemcpy(res.data, buf, sizeof(T) * size,
...
public:
  ...
  struct iterator {
    ...
    __device__ operator bool() const { return idx < graph.size(); }
  };
  ...
  __device__ iterator neighbors_for(size_t idx) {
    size_t first = 0;
    while (first < size() && isinf(distance_between(idx, first)))
      first++;
    return {
        .idx = first,
        .source = idx,
  ...
  __device__ const cu_poly &poly() const { return *polygon; }

  // gets the amount of vertices
  __host__ __device__ size_t size() const { return vertex_count; }
...
public:
  ...
  struct elem {
    ...
    [[nodiscard]] __device__ double estimate() const {
      return until_now + forward;
    }
    // backward cost
    double until_now;
    // forward cost
  ...
  // is the queue empty?
  __device__ bool empty() const { return _used == 0; }
  ...
  __device__ elem &get(size_t idx) { return data[idx].val; }
...
__device__ intersection intersects(const segment &s1, const segment &s2) {
  ...
  if (on_s1 && on_s2) {
    point cross{.x = cross_x, .y = s1.a * cross_x + s1.b};
    return {.is_intersection = true, .pt = cross};
  } else {
    return {.is_intersection = false, .pt = {}};
  }
}
...
__device__ bool point_on(const point &p, const segment &s) {
  if (!between(s.begin.x, p.x, s.end.x))
    return false;
...
__device__ cu_poly::status cu_poly::status_for(const point &p) const {
  for (size_t i = 0; i < boundary.size; i++) {
    if (point_on(p, boundary.data[i]))
      return status::ON_EDGE;
  }

  for (size_t i = 0; i < holes.size; i++) {
    for (size_t j = 0; j < holes.data[i].size; j++) {
      if (point_on(p, holes.data[i].data[j]))
        return status::ON_EDGE;
    }
  }

  if (!inside_single(p, boundary)) {
...
__device__ bool cu_poly::intersects_any(const segment &s) const {
  for (size_t i = 0; i < boundary.size; i++) {
    if (intersects(s, boundary.data[i]).is_intersection)
      return true;
  }

  for (size_t i = 0; i < holes.size; i++) {
    for (size_t j = 0; j < holes.data[i].size; j++) {
      if (intersects(s, holes.data[i].data[j]).is_intersection)
        return true;
    }
  }
...
__device__ segment compute_segment(const point &p1, const point &p2) {
  ...
  return {.begin = p1, .end = p2, .a = tmp, .b = p1.y - tmp * p1.x};
}
...
__device__ point interpolate_from_begin(const segment &s, double delta) {
  if (abs(s.begin.x - s.end.x) < 1e-6) {
    ...
    return {.x = a * (s.begin.y + mod) + b, .y = s.begin.y + mod};
  } else if (s.begin.x < s.end.x) {
    return {.x = s.begin.x + delta, .y = s.a * (s.begin.x + delta) + s.b};
  } else if (s.begin.x > s.end.x) {
    return {.x = s.begin.x - delta, .y = s.a * (s.begin.x - delta) + s.b};
  }
}
...
__global__ void compute_geodesic_distance(uint max_idx, point start,
                                          cu_arr<double> res, cu_graph graph,
                                          size_t idx_offset) {
  uint idx = threadIdx.x + blockDim.x * blockIdx.x + idx_offset;
  if (idx >= max_idx)
    return; // culling
  ...
  for (size_t i = 0; i < graph.size(); i++) {
    ...
    if (edge_viable(e, graph)) {
      ...
      queue.enqueue({.until_now = d, .forward = f, .value = i});
    }
  }
  ...
  while (!queue.empty()) {
    ...
    if (next.forward == 0.0) {
      // found end: distance squared from any point to target = 0? point =
      // target
    ...
    while (iter) {
      ...
      double tentative =
          next.until_now + graph.distance_between(next.value, iter.idx);
      size_t queue_idx = queue.find(iter.idx);
      if (queue_idx == queue.size()) {
        // not yet in queue
        queue.enqueue({.until_now = tentative,
...
__global__ void compute_to_point(uint max_idx, point start, cu_graph graph,
                                 point min, size_t per_row, double granularity,
                                 cu_arr<double> point_dist, cu_res *res,
                                 size_t idx_offset) {
  uint idx = threadIdx.x + blockDim.x * blockIdx.x + idx_offset;
  if (idx > max_idx)
    return; // out of range
  ...
  point target{.x = col_idx * granularity + min.x,
...
__host__ std::optional<result> kernel::operator()(double granularity,
                                                  size_t core_cnt) {
  // alloc buffers & copy data
  std::cout << "[KERNEL]: allocating GPU memory and copying data..."
            << std::endl;
  cu_poly *polygon;
  cu_poly _polygon{.boundary = cu_arr<segment>::copy(data.boundary.size(),
                                                     data.boundary.data()),
                   .holes = cu_arr<cu_arr<segment>>::alloc(data.holes.size())};
  for (size_t i = 0; i < data.holes.size(); i++) {
    _polygon.holes.data[i] =
        cu_arr<segment>::copy(data.holes[i].size(), data.holes[i].data());
  }
  CUDA_SAFE(cudaMallocManaged(&polygon, sizeof(cu_poly)))
  CUDA_SAFE(cudaMemcpy(polygon, &_polygon, sizeof(cu_poly),
                       cudaMemcpyKind::cudaMemcpyDefault))
  cu_graph graph(polygon);

  std::cout
      << "[KERNEL]: step one: starting GPU graph generation (attempting to use "
      << graph.size() << " CUDA threads)..." << std::endl;
  ...
  std::cout << "[KERNEL]: step one finished. GPU graph is generated."
            << std::endl;
  ...
  std::cout << "[KERNEL]: step two: calculate geodesic distances from each "
               "vertex to the start..."
            << std::endl;
  ...
  for (size_t i = 0; i < graph.size(); i += core_cnt) {
    CUDA_KERNEL((compute_geodesic_distance<<<1, core_cnt>>>(
        graph.size(), start, to_vertices, graph, i)))
    CUDA_SAFE(cudaDeviceSynchronize()) // wait till cores are done
  }
  std::cout << "[KERNEL]: step two finished." << std::endl;
  std::cout << "[KERNEL]: cleaning up graph..." << std::endl;
  graph.clean();
  std::cout << "[KERNEL]: outputting geodesic distances to vertices..."
            << std::endl;
  {
    // sub scope for cleanup
    ...
    if (strm.is_open()) {
      ...
      CUDA_SAFE(cudaMemcpy(tmp.data(), to_vertices.data,
  ...
  std::cout << "[KERNEL]: geodesic distances outputted to /tmp/geodesic.csv."
            << std::endl;

  bounding_box box = start_box;
  std::cout << "[KERNEL]: step three: calculate geodesic distances to points..."
            << std::endl;
  auto required_x_steps =
      (size_t)std::ceil((start_box.max.x - start_box.min.x) / granularity);
  auto required_y_steps =
      (size_t)std::ceil((start_box.max.y - start_box.min.y) / granularity);
  cu_res *res;
  CUDA_SAFE(cudaMallocManaged(&res, required_x_steps * required_y_steps *
                                        sizeof(cu_res)))
  std::cout << "[KERNEL]:   requiring " << required_x_steps
            << " steps in x-dimension, and " << required_y_steps
            << " in y-dimension; " << required_x_steps * required_y_steps
            << " steps in total" << std::endl;

  size_t total_steps = required_x_steps * required_y_steps;
  result res_cpu{.distances = std::vector<result::matrix_pt>(total_steps),
                 .farthest = {},
                 .contained = start_box};
  // run kernel
  for (size_t i = 0; i < total_steps; i += core_cnt) {
    CUDA_KERNEL((compute_to_point<<<1, core_cnt>>>(
        total_steps, start, graph, start_box.min, required_x_steps, granularity,
        to_vertices, res, i)))
    CUDA_SAFE(cudaDeviceSynchronize()) // wait for threads to finish
    std::cout << "\r[KERNEL]:   ran " << i << "/" << total_steps << " ("
              << (int)((double)i / (double)total_steps * 100) << "%) steps"
              << std::flush;
  }
  ...
  CUDA_SAFE(cudaMemcpy(distances_cpu.data(), res,
                       distances_cpu.size() * sizeof(cu_res),
                       cudaMemcpyKind::cudaMemcpyDefault))
  double farthest = 0.0;
  for (size_t i = 0; i < distances_cpu.size(); i++) {
    const auto &p = distances_cpu[i];
    res_cpu.distances[i] = {.distance = distances_cpu[i].distance,
  ...
  std::cout << "\n[KERNEL]: step four finished. Farthest point is ("
            << res_cpu.farthest.x << ", " << res_cpu.farthest.y
            << "); distance " << farthest << "." << std::endl;

  std::cout << "[KERNEL]: computations done running. Cleaning up..."
            << std::endl;
  to_vertices.cleanup();
