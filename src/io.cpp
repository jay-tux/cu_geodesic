...
std::vector<segment> read_segm_list(std::istream &strm) {
  ...
  while (!polygon_strm.eof()) {
    ...
    if (!polygon_strm.eof()) {
      ...
      polygon_strm >> c;
      if (c != ';') {
        std::cerr << "[WARNING]: expected ';', got '" << c << "' (ignoring)"
                  << std::endl;
      }
    }
  }
  ...
  for (size_t end = 0; end < points.size(); end++) {
    segment s{.begin = points[end == 0 ? (points.size() - 1) : (end - 1)],
              .end = points[end],
              .a = 0.0,
              .b = 0.0};
    // (y2-y1)/(x2-x1)
...
loader::res_t loader::load(const std::string &filename) {
  ...
  if (!strm.is_open()) {
    std::cerr << "[ERROR]: can't open " << filename << ". Exiting."
              << std::endl;
    std::exit(-1);
  }
  ...
  std::cout << "[MESSAGE]: starting point is (x, y) = (" << start.x << ", "
            << start.y << ")" << std::endl;
  ...
  std::cout << "[MESSAGE]: polygon boundary has " << segm_poly.size()
            << " segments." << std::endl;
  for (const auto &segm : segm_poly) {
    std::cout << "[MESSAGE]:    -> segment(from=(" << segm.begin.x << ","
              << segm.begin.y << "), to=(" << segm.end.x << "," << segm.end.y
              << "), ax+b=" << segm.a << "x+" << segm.b << ")" << std::endl;
  }
  ...
  for (const auto &segm : segm_poly) {
    if (segm.begin.x < xmin)
      xmin = segm.begin.x;
    if (segm.begin.x > xmax)
      xmax = segm.begin.x;
    if (segm.end.x < xmin)
      xmin = segm.end.x;
    if (segm.end.x > xmax)
    ...
    if (segm.begin.y < ymin)
      ymin = segm.begin.y;
    if (segm.begin.y > ymax)
      ymax = segm.begin.y;
    if (segm.end.y < ymin)
      ymin = segm.end.y;
    if (segm.end.y > ymax)
  ...
  bounding_box box{.min = {.x = xmin, .y = ymin},
                   .max = {.x = xmax, .y = ymax}};
  std::cout << "[MESSAGE]: bounding box ranges [xmin, xmax] = [" << xmin << ", "
            << xmax << "] and [ymin, ymax] = [" << ymin << ", " << ymax << "]"
            << std::endl;
  ...
  std::cout << "[MESSAGE]: polygon has " << holes.size() << " holes."
            << std::endl;
  for (size_t hole_id = 0; hole_id < holes.size(); hole_id++) {
    std::cout << "[MESSAGE]:     -> hole #" << hole_id << ": ("
              << holes[hole_id].size() << " segments)" << std::endl;
