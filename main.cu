...
int main(int argc, const char **argv) {
  ...
  auto res = _res.value();
  std::cout << "[MAIN]: using `" << argv[1] << ".csv' as output file." << std::endl;
  std::ofstream dump(std::string(argv[1]) + ".csv");
    ...
    meta << "farthest point: (" << res.farthest.x << ", " << res.farthest.y << ")" << std::endl;
    meta << "bounding box: ((x min, x max), (y min, y max)) = ("
         << "(" << res.contained.min.x << ", " << res.contained.max.x << "), "
         << "(" << res.contained.min.y << ", " << res.contained.max.y << "))" << std::endl;
